...
export class TopLevelCommunityListComponent implements OnInit, OnDestroy {
    ...
    this.currentPageSubscription = observableCombineLatest([pagination$, sort$]).pipe(
      switchMap(([currentPagination, currentSort]) => {
        return this.cds.findTop({
          currentPage: 1,
          elementsPerPage: 100, // Trazer todas as comunidades
          sort: { field: currentSort.field, direction: currentSort.direction },
        });
      }),
    ).subscribe((results) => {
      this.communitiesRD$.next(results);
      if (results?.payload?.page) {
  ...
  /**
   * Remove leading numbers from community name
   */
  ...
  updateScrollButtons() {
    if (this.scrollContainer) {
      ...
      this.showNextButton = container.scrollLeft < (container.scrollWidth - container.clientWidth - 10);
    }
  }
